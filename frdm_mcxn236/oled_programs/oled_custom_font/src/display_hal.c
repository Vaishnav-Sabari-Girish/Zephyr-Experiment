...
void display_hal_set_font_size(uint8_t target_width, uint8_t target_height) {
    ...
    for (int i = 0; i < num_fonts; i++) {
        ...
        /* Must match BOTH width and height now */
        if (f_width == target_width && f_height == target_height) {
            cfb_framebuffer_set_font(display_dev, i);
            return; 
        }
    }
    LOG_WRN("Requested font %dx%d not found!", target_width, target_height);
}
